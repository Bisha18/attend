...
export default function StudentLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!token || role !== "STUDENT") router.push("/");
  }, [router]);
    ...
    <div className="flex h-screen overflow-hidden bg-background text-on-surface">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r-[3px] border-primary h-screen overflow-hidden" style={{ boxShadow: "3px 0 0 #38BDF8" }}>
        ...
        <div className="flex-shrink-0 px-5 pt-5 pb-2">
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-primary/50 mb-3 px-1">Navigation</p>
          {navLinks.map((link) => {
        ...
        <div className="flex-1 flex flex-col justify-between px-4 py-4 min-h-0">
          ...
          <div className="neo-border bg-primary/5 p-4 relative overflow-hidden" style={{ boxShadow: "4px 4px 0px #38BDF8" }}>
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-primary mb-3">// Live GPS Telemetry</p>
            <div className="space-y-2.5">
              {[
                { label: "Signal Lock", val: "98%", w: "98%", color: "bg-primary" },
                { label: "Accuracy", val: "±2m", w: "78%", color: "bg-secondary" },
                { label: "Satellites", val: "12", w: "60%", color: "bg-tertiary" },
              ].map((row) => (
                <div key={row.label}>
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <span className="text-[9px] font-bold text-green-600 uppercase">All relays nominal</span>
            </div>
          </div>

          {/* Decorative card */}
          <div className="mt-4 border-[3px] border-dashed border-primary/30 p-3 bg-primary-container/40 relative overflow-hidden">
            <span className="material-symbols-outlined text-[56px] absolute -right-2 -bottom-2 text-primary/10" style={{ fontVariationSettings: "'FILL' 1" }}>radar</span>
            <p className="text-[9px] font-black uppercase text-primary/40 tracking-widest">Session Active</p>
            <p className="text-xs font-black text-on-surface mt-1">Student clearance<br />level operative.</p>
          </div>
        </div>

        {/* Pinned Bottom */}
        <div className="flex-shrink-0 border-t-[3px] border-primary">
          <div className="bg-primary-container px-4 py-2.5 flex items-center gap-2 border-b-[2px] border-primary/30">
            <span className="w-2 h-2 rounded-full bg-green-500"></span>
            <p className="text-[10px] font-black uppercase text-primary tracking-wider">System: Online</p>
          </div>
          <button
        ...
        <header className="flex-shrink-0 sticky top-0 z-40 bg-white border-b-[3px] border-primary flex justify-between items-center px-4 sm:px-6 h-16 sm:h-20" style={{ boxShadow: "0 3px 0 #38BDF8" }}>
          ...
          <div className="hidden lg:flex items-center gap-2">
            <div className="bg-primary/10 text-primary px-3 py-1.5 font-bold text-xs uppercase flex items-center gap-2 neo-border neo-shadow-sm">
              <span className="material-symbols-outlined text-[14px] text-green-600">sensors</span>
              GPS Active
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-[9px] font-bold text-primary/60 uppercase tracking-widest">Student</p>
            </div>
